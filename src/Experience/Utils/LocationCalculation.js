...
export default class LocationCalculation {
    ...
    cal3Matrix(position) {
        ...
        if (position.x > 0 && position.z > 0) {
            let positionNew01 = new THREE.Vector3(position.x - 0.5 * offset_num, position.y, position.z - 2.5 * offset_num)
            let matrixNew01 = new THREE.Matrix4()

            matrixNew01.setPosition(positionNew01)
            matrices.push(matrixNew01)

            let positionNew02 = new THREE.Vector3(position.x - 2.5 * offset_num, position.y, position.z - 0.5 * offset_num)
            let matrixNew02 = new THREE.Matrix4()
            matrixNew02.setPosition(positionNew02)
            matrices.push(matrixNew02)

            let positionNew03 = new THREE.Vector3(position.x - 2.5 * offset_num, position.y, position.z - 2.5 * offset_num)
            ...
            let positionNew01 = new THREE.Vector3(position.x - 2.5 * offset_num, position.y, position.z + 0.5 * offset_num)
            let matrixNew01 = new THREE.Matrix4()
            matrixNew01.setPosition(positionNew01)
            matrices.push(matrixNew01)

            let positionNew02 = new THREE.Vector3(position.x - 0.5 * offset_num, position.y, position.z + 2.5 * offset_num)
            let matrixNew02 = new THREE.Matrix4()
            matrixNew02.setPosition(positionNew02)
            matrices.push(matrixNew02)
